...
const fetchWebUserTktkSigRaw = async (client, signedURL, retryCount = 0) => {
  try {
    ...
    if (data.statusCode !== 0) {
      if (data.statusCode === 10202) {
        await logWarning(
          `handleFetchLeads - fetchWebUserTktkSigRaw, user does not exist for ${signedURL}`
        );
      } else {
        await logError(
          `handleFetchLeads - fetchWebUserTktkSigRaw, missing signature for url ${signedURL}`
        );
      }
    ...
    if (errorCodes.includes(error.code) && retryCount < MAX_RETRY_COUNT) {
      await logWarning(
